...
interface EditBookFormInputs {
  [key: string]: string | EditBookFormInputs;
  title: string;
  author: string;
  authorId: string;
  genre: string;
...
function trimObjectProperties(
  formData: EditBookFormInputs
): EditBookFormInputs {
  return Object.keys(formData).reduce(
    (trimmedData: EditBookFormInputs, key: string) => {
...
export const EditBook = () => {
  ...
  if (authorId !== user.id) {
    return <div className="">You are not authorized!</div>;
  }
  const onSubmit = (formData: EditBookFormInputs) => {
    const payload = {
      id: bookIdDb,
      data: formData,
    };
          ...
          <div className="relative mx-auto max-w-[525px] overflow-hidden rounded-lg bg-white py-16 px-10 text-center sm:px-12 md:px-[60px]">
            ...
            >
              ...
              {/* <datalist id="book-genre-datalist">
                {BOOK_GENRE_LIST.map((genreX, idx) => (
                  <option key={idx + 1} value={genreX}>
                    {genreX}
                  </option>
                ))}
              </datalist> */}
              ...
              >
                ...
                {BOOK_GENRE_LIST.map((newGenre) => (
                  <option key={newGenre} value={newGenre}>
                    {newGenre}
                  </option>
                ))}
              </select>
